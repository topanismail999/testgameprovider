...
const PROMOS = [
  { id: 1, title: "WELCOME BONUS 100%", sub: "Khusus Member Baru - Klaim Sekarang!", color: "from-blue-600 to-indigo-900" },
  { id: 2, title: "REBATE HARIAN 0.8%", sub: "Tanpa Batas Rollingan Setiap Hari", color: "from-purple-600 to-fuchsia-900" },
  { id: 3, title: "EVENT PETIR ZEUS", sub: "Extra Perkalian s/d x500 Menanti Anda", color: "from-amber-500 to-red-800" },
];
...
export default function GameApp() {
  ...
  // Efek Auto-slide untuk Banner Promo
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-200 font-sans pb-20 selection:bg-yellow-500 selection:text-black">
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex justify-between items-center">
          ...
          <div className="flex gap-4 items-center">
            ...
            <div className="bg-gradient-to-r from-slate-900 to-slate-800 px-4 py-2 rounded-2xl border border-white/10 shadow-inner">
              <p className="text-[8px] text-slate-500 font-black uppercase leading-none mb-1 text-center">Saldo Anda</p>
      ...
      <div className="max-w-7xl mx-auto px-6 mt-6">
        <div className={`w-full h-48 md:h-60 rounded-[2.5rem] p-8 md:p-12 relative overflow-hidden transition-all duration-700 bg-gradient-to-br ${PROMOS[currentPromo].color} shadow-2xl`}>
          <div className="relative z-10 h-full flex flex-col justify-center">
            ...
            <h2 className="text-3xl md:text-5xl font-black text-white italic leading-none uppercase tracking-tighter">
            ...
            <button className="mt-6 bg-white text-black text-[10px] font-black uppercase px-6 py-3 rounded-xl w-max hover:bg-yellow-500 transition-colors shadow-xl active:scale-95">
              Klaim Sekarang
            </button>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 mt-12">
        <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex gap-2">
               <div className="px-3 py-1 bg-white/5 rounded-lg border border-white/10 text-[9px] font-black uppercase text-emerald-400 flex items-center gap-2">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
          {GAMES.map((game) => (
            <div key={game.id} onClick={() => launchGame(game.id)} className="group cursor-pointer">
              <div className="relative aspect-[3/4] rounded-[2.5rem] bg-[#0f172a] border border-white/5 overflow-hidden shadow-xl transition-all duration-500 group-hover:border-yellow-500/50 group-hover:-translate-y-2 group-hover:shadow-yellow-500/10">
                ...
                {/* RTP BANNER WIDGET */}
                <div className="absolute bottom-0 left-0 w-full p-4 bg-black/60 backdrop-blur-md border-t border-white/10">
                ...
                {/* Hot Tag */}
                {game.hot && (
                  <div className="absolute top-4 right-4 bg-gradient-to-r from-red-600 to-orange-600 text-[8px] font-black px-2 py-0.5 rounded-full text-white uppercase italic shadow-lg animate-bounce">Hot</div>
                )}
                ...
                <div className="absolute inset-0 bg-yellow-500/10 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <div className="w-14 h-14 bg-yellow-500 text-black rounded-full flex items-center justify-center shadow-2xl scale-50 group-hover:scale-100 transition-all duration-300">
                       <svg className="w-6 h-6 fill-current" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
                    </div>
                </div>
              </div>
              <div className="mt-4 text-center">
                <h4 className="text-[11px] font-black text-slate-300 group-hover:text-yellow-500 uppercase tracking-tighter transition-colors px-2 truncate">{game.name}</h4>
      ...
      {/* --- GAME MODAL IFRAME --- */}
      {selectedGameUrl && (
        <div className="fixed inset-0 z-[100] bg-black flex flex-col transition-all duration-500">
          <div className="h-14 bg-[#0f172a] flex justify-between items-center px-4 border-b border-white/10">
            <div className="flex items-center gap-2">
               <div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></div>
               <p className="text-[10px] font-black text-yellow-500 tracking-tighter uppercase italic">Secure Connection: Nexus Engine</p>
            </div>
            <button 
               onClick={() => setSelectedGameUrl(null)} 
               className="bg-red-600 hover:bg-red-700 text-white px-6 py-1.5 rounded-full text-[10px] font-black uppercase transition-all active:scale-90"
            >
               Keluar Game
            </button>
          </div>
      ...
      {isLoading && (
        <div className="fixed inset-0 z-[110] bg-[#020617]/95 backdrop-blur-md flex flex-col items-center justify-center">
           <div className="w-12 h-12 border-4 border-yellow-500 border-t-transparent rounded-full animate-spin mb-4 shadow-[0_0_20px_rgba(234,179,8,0.2)]"></div>
           <p className="text-[10px] font-black text-yellow-500 uppercase tracking-[0.4em] animate-pulse">Menghubungkan Server...</p>
        </div>
      )}
